...
export const useLessonLogicViet = ({
  selectedClass,
  selectedSubject,
  activeTopic,
  activeLesson,
  ...
  const [textPassage, setTextPassage] = useState(""); // ✅ thêm để lưu đoạn văn đọc hiểu
  ...
  const handleLevelSelect = async (level, forceRestart = false) => {
    ...
    try {
      ...
      const text = data.textPassage || "";

      // ❗ Nếu không có câu hỏi → báo và dừng
  ...
  const loadQuizFromStart = async (level) => {
    ...
    try {
      ...
      const data = qsRes.data.data || {};
      const allQuestions = data.questions || data || [];
      setQuestions(allQuestions);
      setTextPassage(data.textPassage || "");
      setUserAnswers({});
