...
const QuestionComponent = ({ 
  ...
  const convertMathSymbols = (text) => {
    ...
    t = t.replace(/[xX]/g, (m, offset, str) => {
      ...
      // Nếu kề bên chữ ⇒ là chữ thật (xếp, xe, …)
      if (isPrevLetter || isNextLetter) return m;

      // Nếu kề bên số ⇒ dạng toán học 1x hoặc x1 ⇒ nhân
      if (isPrevDigit || isNextDigit) return " nhân ";

      // ⭐ Trường hợp biến x (không kề chữ, không kề số)
      // Ví dụ: "x - 327 = 568"
      return " ích ";
    });
    ...
    t = t.replace(/([-])/g, (match, symbol, offset, str) => {
      const prev = str[offset - 1];
      const next = str[offset + 1];
      ...
      // Nếu nằm giữa CHỮ → không thay (từ ghép)
      if (isPrevLetter && isNextLetter) return match;

      // Nếu nằm giữa số → trừ
      if (isPrevDigit && isNextDigit) return " trừ ";

      // Nếu 1 bên là số 1 bên không → có thể là biểu thức dạng "x - 5"
      if (isPrevDigit || isNextDigit) return " trừ ";
