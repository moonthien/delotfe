...
const ProfileContent = ({
  ...
  const recentChartData = [...filteredExamHistory]
    ...
    .map((r, idx) => ({
      ...
      subject: r.refId?.subjectId?.name || r.refId?.topicId?.subjectId?.name || r.refId?.topicId?.name || "Không rõ",
      period: r.refType === "Lesson" ? "Ôn tập" : r.refId?.period || r.refId?.name || "Không rõ",
  ...
  useEffect(() => {
    const fetchAllQuestions = async () => {
      try {
        const lessonIds = [...new Set(filteredExamHistory.map(r => r.refId?._id))];
        let map = {};  // Đổi tên thành questionMap để rõ
        for (const lessonId of lessonIds) {
          const type = filteredExamHistory.find(r => r.refId?._id === lessonId)?.refType;
          const url =
            type === "Lesson"
              ? `http://localhost:3999/v1/api/questions/lesson/${lessonId}`
              : `http://localhost:3999/v1/api/questions/exam/${lessonId}`;
      ...
      return {
        ...
        type:
          parentResult?.refType === "Lesson"
            ? "Ôn tập"
            : (parentResult?.refId?.period || parentResult?.refId?.name || "Không rõ")
      };
    })
  ...
  const donutColors = [
    "#e74c3c", "#ff7675", "#fab1a0", "#ffeaa7", "#fd79a8",
    "#74b9ff", "#55efc4", "#a29bfe", "#fdcb6e", "#81ecec"
  ];

  // Helper functions for table data
  const getSubjectName = (result) => {
    return result.refId?.subjectId?.name || result.refId?.topicId?.subjectId?.name || result.refId?.topicId?.name || "Không rõ";
  };

  const getTypeName = (result) => {
    return result.refType === "Lesson" ? "Ôn tập" : (result.refId?.period || result.refId?.name || "Không rõ");
  };
    ...
    <Content>
          {activeSection === "profile" && (
            <>
              <HeaderInfo>
                ...
                {!isEditing ? (
                  <UpdateBtn onClick={() => setIsEditing(true)}> Cập nhật ✏️ </UpdateBtn>
                ) : (
                  <div style={{ display: "flex", gap: "10px" }}>
                    <UpdateBtn onClick={() => setIsEditing(false)}>Quay lại</UpdateBtn>
            ...
            <>
                      ...
                      <StatValue>
                        <CountUp
                            ...
                            filteredExamHistory.length > 0
                              ? Math.round(
                                  filteredExamHistory.reduce((sum, r) => sum + r.score, 0) / filteredExamHistory.length
                                )
                              : 0
                          }
                          duration={2200}
                      ...
                      <>
                        ...
                        >
                          <StatCard>
                            ...
                            <StatValue>
                              {(() => {
                                const ontap = filteredExamHistory.filter(r => r.refType === "Lesson");
                                const avg = ontap.length > 0
                                  ? Math.round(ontap.reduce((sum, r) => sum + r.timeSpent, 0) / ontap.length)
                                  : 0;
                                  return <CountUp end={avg} duration={2000} startWhen={true} suffix=" giây" />;
                              })()}
                            </StatValue>
                          </StatCard>
                        </motion.div>
                      ...
                      <>
                        ...
                        >
                          <StatCard>
                            ...
                            <StatValue>
                              {(() => {
                                const kt = filteredExamHistory.filter(r => r.refType !== "Lesson");
                                const avg = kt.length > 0
                                  ? Math.round(kt.reduce((sum, r) => sum + r.timeSpent, 0) / kt.length)
                                  : 0;
                                return <CountUp end={avg} duration={2000} startWhen={true} suffix=" giây" />;
                              })()}
                            </StatValue>
                    ...
                    <thead>
                      <tr>
                        <th onClick={() => handleSort('subject')}>
                          ...
                          {sortConfig.key === 'subject' && sortConfig.direction === 'desc' && ' ↓'}
                          </th>
                        ...
                        <th onClick={() => handleSort('correct')}>
                          Đúng
                          {sortConfig.key === 'correct' && sortConfig.direction === 'asc' && ' ↑'}
                          {sortConfig.key === 'correct' && sortConfig.direction === 'desc' && ' ↓'}
                        </th>
                        <th onClick={() => handleSort('wrong')}>
                          Sai
                          {sortConfig.key === 'wrong' && sortConfig.direction === 'asc' && ' ↑'}
                          {sortConfig.key === 'wrong' && sortConfig.direction === 'desc' && ' ↓'}
                        </th>
                        <th onClick={() => handleSort('time')}>
                          Thời gian
                          {sortConfig.key === 'time' && sortConfig.direction === 'asc' && ' ↑'}
                          {sortConfig.key === 'time' && sortConfig.direction === 'desc' && ' ↓'}
                        </th>
                      ...
                      {currentExam.map((result, index) => {
                          ...
                          return (
                            <tr key={result._id}>
                              ...
                              >
                                <div className="scroll-text">
                                  {result.refType === "Lesson" ? (
                                    <>
                                      {result.refId?.order && ` - Bài ${result.refId.order}: `}
                                      {result.refId?.title || "Không rõ tên bài"}
                                      {" - "}
                                      <LevelBadge level={result.level || "trungbinh"}>
                                        {result.level === "de" ? "Dễ" :
                                          result.level === "trungbinh" ? "Trung bình" :
                                          result.level === "nangcao" ? "Nâng cao" :
                                          "Không rõ"}
                                      </LevelBadge>
                              ...
                              <td>
                                <CorrectText>{correct}/{totalQuestions}</CorrectText>
                              </td>
                              <td>
                                <WrongText>{wrong}/{totalQuestions}</WrongText>
                              </td>
                              <ScrollTextCell
                                style={{ maxWidth: "80px" }}
                                ref={(el) => {
              ...
              {filteredExamHistory.length > 0 && (
                <motion.div
                  ...
                  style={{ margin: "20px 0", background: "#fff", borderRadius: "12px", padding: "16px", boxShadow: "0 2px 10px rgba(0,0,0,0.05)" }}
                >
                  <h4 style={{ margin: "0 0 12px 0", color: "#2f9d2f", fontSize: "15px", fontWeight: "600" }}>
                      ...
                      <BarChart
                        data={filteredExamHistory.map((r, idx) => ({
                          name:
                            r.refType === "Lesson"
                              ? `Ôn tập ${idx + 1}`
                              : r.refId?.period || r.refId?.name || `Bài ${idx + 1}`,
                          ...
                          <Pie
                            data={[
                              ...
                              },
                              {
                                ...
                                value: filteredExamHistory.reduce(
                                  (sum, r) =>
                                    sum + (r.answers.length - (r.correctAnswers || 0)),
                                    0
                                  ),
                              },
                            ]}
          ...
          {activeSection === "stats" && (
            <>
              ...
              <FilterContainer style={{ alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
                    ...
                    {showWrongQuestions ? (
                      ...
                      <>
                        {topWrongQuestions.length > 0 ? (
                          <>
                            ...
                            >
                              ...
                              <ResponsiveContainer width="100%" height={300}>
                                <BarChart data={topWrongQuestions} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
                                  ...
                                  <XAxis
                                    ...
                                    height={80}
                                    tickFormatter={(id) => {
                                      const text = questionTextMap[id]?.text || "Không có nội dung";
                                  ...
                                  <Tooltip
                                    ...
                                    labelFormatter={(id) => {
                                      const q = topWrongQuestions.find(t => t.questionId === id);
                                      return q?.text || "Không rõ";
                                    }}
                                    contentStyle={{ borderRadius: "8px", border: "none", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}
                                  />
                                  <Bar dataKey="count" fill="#e74c3c" radius={[8, 8, 0, 0]} />
                                </BarChart>
                              </ResponsiveContainer>
                            </motion.div>
                            ...
                            >
                              <h4 style={{ marginBottom: 12, color: "#e74c3c" }}>Tỷ lệ sai của Top 10 câu hỏi</h4>
                                ...
                                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
                                  <thead>
                                    <tr style={{ background: "#fdf2f2" }}>
                                      <th style={{ padding: "12px", textAlign: "left", borderBottom: "2px solid #e74c3c"}}>Môn học</th>
                                      <th style={{ padding: "12px", textAlign: "center", borderBottom: "2px solid #e74c3c"}}>Loại bài</th>
                                      <th style={{ padding: "12px", textAlign: "center", borderBottom: "2px solid #e74c3c"}}>Câu hỏi</th>
                                      <th style={{ padding: "12px", textAlign: "center", borderBottom: "2px solid #e74c3c" }}>Lần sai</th>
                                      <th style={{ padding: "12px", textAlign: "center", borderBottom: "2px solid #e74c3c", width: "100px" }}>Đáp án chọn</th>
                                      <th style={{ padding: "12px", textAlign: "center", borderBottom: "2px solid #e74c3c" }}>Đáp án đúng</th>
                                      <th style={{ padding: "12px", textAlign: "center", borderBottom: "2px solid #e74c3c", width: "100px"}}>Độ nguy hiểm</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    {topWrongQuestions.map((q, idx) => (
                                      <tr key={q.questionId} style={{ background: idx % 2 === 0 ? "#fdfafa" : "#fff" }}>
                                        <td style={{ padding: "12px", minWidth: "110px", width: "110px" }}>{q.subject}</td>
                                        <td style={{ padding: "12px", fontSize: "13.5px", minWidth: "180px" }}>
                                          {(() => {
                                            // Tìm kết quả chứa câu hỏi này
                                            const parentResult = filteredExamHistory.find(r =>
                                              r.answers.some(a => a.questionId === q.questionId)
                                            );
                                            if (!parentResult) return <span style={{color:"#95a5a6"}}>Không rõ</span>;

                                            if (parentResult.refType === "Lesson") {
                                              // === CASE 1: BÀI ÔN TẬP ===
                                              const topic = parentResult.refId?.topicId;
                                              const lesson = parentResult.refId;

                                              const topicName = topic?.name || topic?.title || "Ôn tập";
                                              const lessonOrder = lesson?.order
                                                ? `Bài ${lesson.order}`
                                                : lesson?.title?.match(/^Bài\s*\d+/)
                                                  ? lesson.title.split(":")[0].trim()
                                                  : "Bài luyện tập";
                                              const lessonTitle = lesson?.title || "Không có tiêu đề";
                                              const level = parentResult.level || lesson?.level;
                                              const levelText = level === "de" ? "Dễ" :
                                                                level === "trungbinh" ? "Trung bình" :
                                                                level === "nangcao" ? "Nâng cao" : "";
                                              const levelColor = level === "de" ? "#27ae60" :
                                                                level === "trungbinh" ? "#f39c12" : "#e74c3c";
                                              const levelBg = level === "de" ? "#d5f5e3" :
                                                                level === "trungbinh" ? "#fef9e7" : "#fadadd";
                                              ...
                                              return (
                                                <div style={{ lineHeight: "1.45" }}>
                                                  ...
                                                  <div style={{ color: "#34495e", fontSize: "14px" }}>
                                                    {lessonOrder}: {lessonTitle.replace(/^Bài\s*\d+[:\-\s]*/, "") /* bỏ lặp "Bài XX" nếu có */}
                                                    {levelText && (
                                                      <span style={{
                                                        marginLeft: "8px",
                                                        padding: "3px 9px",
                                                  ...
                                                  {parentResult.isAIPractice && (
                                                    <div style={{ color: "#8e44ad", fontSize: "14px", marginTop: "3px", fontWeight: "500" }}>
                                                      AI Practice
                                                    </div>
                                                  )}
                                                </div>
                                              ...
                                              // === CASE 2: BÀI KIỂM TRA ===
                                              const examName = parentResult.refId?.period || parentResult.refId?.name || "Kiểm tra";
                                              return (
                                                <span style={{ color: "#2980b9", fontWeight: "600", fontSize: "14px" }}>
                                                  Kiểm tra {examName}
                                                </span>
                                              );
                                            }
                                        ...
                                        >
                                          <div className="scroll-text">{q.text}</div>
                                        </ScrollTextCell>
                                        <td style={{ padding: "12px", textAlign: "center", fontWeight: "bold", color: "#e74c3c" }}>{q.count}</td>
                                        <td style={{ padding: "12px", textAlign: "center" }}>{q.chosenAnswers.length > 0 ? q.chosenAnswers.join(", ") : "-"}</td>
                                        <td style={{ padding: "12px", textAlign: "center", color: "#2f9d2f", fontWeight: "600" }}>{q.correctAnswer || "N/A"}</td>
                                        <td>
                                          {q.count >= 5 ? "⚠️ Rất nguy hiểm" :
                                           q.count >= 3 ? "🟠 Lưu ý" : "🟡 Sai nhẹ"}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
                          ...
                          <Message style={{ color: "#e74c3c", fontSize: "16px" }}>Chưa có câu nào bị sai nhiều lần</Message>
                        )}
                      </>
                    ) : (
                        ...
                        <ResponsiveContainer width="100%" height={350}>
                          {chartType === "bar" ? (
                            <BarChart data={recentChartData}>
                              ...
                              <Tooltip
                                ...
                                labelFormatter={(label, payload) => {
                                  if (payload && payload.length > 0) {
                                    const { date, subject, period, correctAnswers, totalQuestions, timeSpent } = payload[0].payload;
                                    return (
                                      <div style={{ fontSize: '12px', padding: '8px', background: 'white', borderRadius: '8px' }}>
                                        <strong>{label}</strong><br />
                                        Ngày: {date}<br />
                                        Môn: {subject}<br />
                                        Kỳ: {period}<br />
                                        Đúng: {correctAnswers}/{totalQuestions}<br />
                                        Thời gian: {timeSpent}
                                      </div>
                                    );
                                  }
                              ...
                              />
                              <Legend />
                              <Bar dataKey="score" fill="#2f9d2f" radius={[8, 8, 0, 0]} />
                            </BarChart>
                          ) : (
                            <LineChart data={recentChartData}>
                              <CartesianGrid strokeDasharray="3 3" />
                              <XAxis dataKey="name" interval={0} angle="end" textAnchor="end" tick={{ fontSize: 12 }} />
                              <YAxis domain={[0, 100]} />
                              <Tooltip formatter={(value) => `${value} điểm`} />
                              <Line type="monotone" dataKey="score" stroke="#2f9d2f" strokeWidth={3} dot={{ fill: "#2f9d2f", r: 6 }} />
                            </LineChart>
                          )}
                        </ResponsiveContainer>
                          ...
                          <tbody>
                            {[...filteredExamHistory]
                              ...
                              .map((result, idx) => (
                                <tr key={result._id}>
                                  <td>{idx + 1}</td>
                                  <td>{result.refId?.subjectId?.name || result.refId?.topicId?.subjectId?.name || "Không rõ"}</td>
                                  <td>{result.refType === "Lesson" ? "Ôn tập" : (result.refId?.period || result.refId?.name || "Không rõ")}</td>
                                  <td><strong>{result.score}</strong></td>
                                  <td>{result.correctAnswers}/{result.answers.length}</td>
                                  <td>{Math.floor(result.timeSpent / 60)} phút {result.timeSpent % 60} giây</td>
                                  <td>{formatDate(result.createdAt)}</td>
                                </tr>
                            ))}
                          </tbody>
